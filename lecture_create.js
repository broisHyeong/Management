    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            professors.forEach((professor, index) => {
                console.log(`${index + 1}. ${professor.professor_name} - ${professor.professor_id} - ${professor.professor_major}`);
            });
...
function getProfessorList() {
    return new Promise((resolve, reject) => {
        const sql = 'SELECT professor_id, professor_name, professor_tel, professor_major FROM professor';
...
async function addLectureToDatabase() {
    try {
        ...
        let sql = 'INSERT INTO lecture(lecture_id, lecture_name, lecture_day, lecture_time, lecture_credit, professor_id, lecture_type, major_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
